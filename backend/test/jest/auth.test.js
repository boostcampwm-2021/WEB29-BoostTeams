...
describe('Auth 테스트', () => {
	beforeAll(async () => {
		...
		await request(app)
			...
			.send({
				...
				encryptedPassword: getEncryptedPw('test123test123')
			});
	}, 30000);
	...
	test('로그인 비밀번호 불일치', async () => {
		const res = await request(app)
			...
			.send({
				userEmail: 'test@naver.com',
				encryptedPassword: getEncryptedPw('test123test1')
			});
...
const getEncryptedPw = (pw) => AES.encrypt(pw, process.env.AES_KEY).toString();
